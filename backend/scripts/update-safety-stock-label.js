#!/usr/bin/env node
/**
 * Update Safety Stock field label in the view
 */
...
async function main() {
  console.log('=== Update Safety Stock Label ===\n');

  const uid = await authenticate();

  // Updated arch with new label
  const archXml = `<?xml version="1.0"?>
<data>
  <xpath expr="//field[@name='country_of_origin']" position="after">
    <field name="x_safety_stock" string="Safety Stock (FBM/FBR)"/>
  </xpath>
</data>`;
  ...
  console.log('Updating view 4821 with new label...');
  ...
  if (updateResult.includes('True') || updateResult.includes('1')) {
    console.log('✓ Label updated to "Safety Stock (FBM/FBR)"');
