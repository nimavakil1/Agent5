...
async function main() {
  ...
  // Find product.template.common.form or product.template.product.form
  ...
  // Use product.template.product.form as the parent (it's the main product form)
  const parentView = baseViews.find(v => v.name === 'product.template.product.form');

  if (!parentView) {
    console.log('\nCould not find product.template.product.form, searching by ID 434...');
    const v434 = await odoo.searchRead('ir.ui.view', [['id', '=', 434]], ['id', 'name', 'arch']);
    if (v434.length > 0) {
      console.log('Found:', v434[0].name);
    }
  }
  ...
  const existingCustom = await odoo.searchRead('ir.ui.view',
    [['name', '=', 'product.template.form.hamid.fxxl']],
    ['id']
  );
  ...
  // Create inherited view with new tab showing all images
  const archXml = `<?xml version="1.0"?>
<data>
  <xpath expr="//page[@name='inventory']" position="after">
    <page string="Hamid-FXXL" name="hamid_fxxl">
      <group>
  ...
  console.log('Tab "Hamid-FXXL" has been added to ALL product.template forms!');
