#!/usr/bin/env node
/**
 * Make x_safety_stock field read-only in Odoo
 * Users will manage it from Agent5 UI instead
 */
...
async function main() {
  console.log('=== Make Safety Stock Read-Only in Odoo ===\n');

  const uid = await authenticate();

  // Update the view to make the field readonly with a helpful tooltip
  const archXml = `<?xml version="1.0"?>
<data>
  <xpath expr="//field[@name='country_of_origin']" position="after">
    <field name="x_safety_stock" string="Safety Stock (FBM/FBR)" digits="[12, 0]" readonly="1" help="Managed via Agent5. This value is deducted from free stock when syncing to Amazon FBM/FBR."/>
  </xpath>
</data>`;
  ...
  console.log('Updating view to make field read-only...');
  ...
  if (updateResult.includes('True') || updateResult.includes('1')) {
    console.log('✓ Field is now READ-ONLY in Odoo');
    console.log('✓ Added tooltip: "Managed via Agent5"');
