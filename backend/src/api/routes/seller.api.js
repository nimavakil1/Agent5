...
let sellerReviewsSync = null;

async function getReviewsSync() {
  if (!sellerReviewsSync) {
    const { getSellerReviewsSync } = require('../../services/amazon/seller');
    sellerReviewsSync = await getSellerReviewsSync();
  }
  return sellerReviewsSync;
}

/**
 * @route GET /api/seller/reviews
 * @desc Get seller reviews with filters
 * @query rating - Filter by specific rating (1-5)
 * @query minRating - Filter by minimum rating
 * @query maxRating - Filter by maximum rating
 * @query marketplace - Filter by marketplace (DE, FR, etc.)
 * @query orderId - Search by order ID
 * @query hasComments - Only show reviews with comments
 * @query dateFrom - Filter by date from
 * @query dateTo - Filter by date to
 * @query limit - Max results (default 50)
 * @query skip - Skip N results (pagination)
...
router.get('/reviews', async (req, res) => {
  try {
    ...
    const [reviews, total] = await Promise.all([
      reviewsSync.getReviews(filters, options),
      reviewsSync.countReviews(filters)
    ]);
    ...
    console.error('[SellerAPI] GET /reviews error:', error);
    res.status(500).json({ success: false, error: error.message });
  }
});
...
router.get('/reviews/stats', async (req, res) => {
  try {
    const reviewsSync = await getReviewsSync();
    const stats = await reviewsSync.getStats();
    ...
    console.error('[SellerAPI] GET /reviews/stats error:', error);
    res.status(500).json({ success: false, error: error.message });
  }
});

/**
 * @route GET /api/seller/reviews/status
 * @desc Get reviews sync status
 */
router.get('/reviews/status', async (req, res) => {
  try {
    const reviewsSync = await getReviewsSync();
    const status = await reviewsSync.getStatus();
    ...
    console.error('[SellerAPI] GET /reviews/status error:', error);
    res.status(500).json({ success: false, error: error.message });
  }
});

/**
 * @route POST /api/seller/reviews/sync
 * @desc Trigger reviews sync (request report + process pending)
 */
router.post('/reviews/sync', async (req, res) => {
  try {
    const reviewsSync = await getReviewsSync();
    const result = await reviewsSync.sync();
    ...
    console.error('[SellerAPI] POST /reviews/sync error:', error);
    res.status(500).json({ success: false, error: error.message });
  }
});

/**
 * @route GET /api/seller/reviews/order/:orderId
 * @desc Get review for a specific order
 */
router.get('/reviews/order/:orderId', async (req, res) => {
  try {
    const reviewsSync = await getReviewsSync();
    const review = await reviewsSync.getReviewByOrder(req.params.orderId);

    if (!review) {
      return res.status(404).json({ success: false, error: 'No review found for this order' });
    }
    ...
    console.error('[SellerAPI] GET /reviews/order/:orderId error:', error);
    res.status(500).json({ success: false, error: error.message });
  }
});

/**
 * @route GET /api/seller/reviews/product/:asin
 * @desc Get reviews for a specific product (by ASIN)
 */
router.get('/reviews/product/:asin', async (req, res) => {
  try {
    const reviewsSync = await getReviewsSync();

    const [reviews, summary] = await Promise.all([
      reviewsSync.getReviewsByProduct(req.params.asin, { limit: parseInt(req.query.limit) || 20 }),
      reviewsSync.getProductReviewSummary(req.params.asin)
    ]);
    ...
    console.error('[SellerAPI] GET /reviews/product/:asin error:', error);
    res.status(500).json({ success: false, error: error.message });
  }
});
