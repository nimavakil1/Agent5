...
async function checkUnlinkedInvoices() {
  ...
  // Find ALL Amazon seller orders with invoice_status = "to invoice" (no limit)
  console.log('Finding ALL Amazon seller orders with invoice_status = "to invoice"...');
  ...
  for (const partnerId of partnerIds) {
    ...
    checkedPartners++;
    if (checkedPartners % 100 === 0) {
      console.log('Checked ' + checkedPartners + '/' + partnerIds.length + ' partners... (found ' + potentialMatches.length + ' matches so far)');
    }
  ...
  // Sort by date
  potentialMatches.sort((a, b) => (a.order.date || '').localeCompare(b.order.date || ''));
  ...
  for (const match of potentialMatches) {
    ...
    console.log('  Order Date: ' + (match.order.date ? match.order.date.substring(0, 10) : 'N/A'));
